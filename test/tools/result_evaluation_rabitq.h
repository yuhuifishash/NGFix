...
SearchResult TestSingleQuery(float* query_data, int* gt, size_t k, size_t efs, HNSW_NGFix_RaBitQ* searcher) {
    size_t ndc = 0;
...
void TestQueries(std::ostream& s, float* test_query, int* test_gt, size_t test_number, size_t k,
                            size_t test_gt_d, size_t vecdim, HNSW_NGFix_RaBitQ* searcher)
{
    //output header
    ...
    for(auto efs : efss) {
        ...
        for(int i = 0; i < test_number; ++i){
            ...
            auto res = TestSingleQuery(test_query+1ll*i*vecdim, gt, k, efs, searcher);
